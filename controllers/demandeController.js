...
const traiterDemande = async (req, res) => {
  const { id } = req.params;
  const { statut } = req.body; // Reçu du frontend
  try {
    // Note : ta table utilise la colonne "staut" (sans 'e') comme vu dans le dump SQL
    const query =
      "UPDATE demandes SET staut = $1 WHERE id_demande = $2 RETURNING *";
    const result = await pool.query(query, [statut, id]);
    if (result.rows.length === 0)
      return res.status(404).json({ message: "Demande introuvable" });
    res.json({
      message: `Demande passée au statut : ${statut}`,
...
// [PUT] Mettre à jour (Modifier) intégralement une demande existante
const updateDemande = async (req, res) => {
  ...
  try {
    const query = `
            UPDATE demandes 
            SET titre_demande = $1, description = $2, id_projet = $3, id_employe = $4
            WHERE id_demande = $5 AND is_delete = false
            RETURNING *`;
    const result = await pool.query(query, [
      titre_demande,
      description,
    ...
    if (result.rows.length === 0) {
      return res
        .status(404)
        .json({ error: "Demande introuvable ou archivée dans la corbeille." });
    }
...
const restaurerDemande = async (req, res) => {
  ...
  try {
    ...
    if (result.rows.length === 0)
      return res
        .status(404)
        .json({ message: "Demande introuvable dans la corbeille." });
